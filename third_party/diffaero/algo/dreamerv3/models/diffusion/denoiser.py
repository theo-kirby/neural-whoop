...
class Denoiser(nn.Module):
    ...
    def forward(self, noisy_next_obs: Tensor, sigma: Tensor, obs: Tensor, act: Tensor,drone_states:Tensor,obstacles:Tensor) -> Tuple[Tensor, Tensor]:
        c_in, c_out, c_skip, c_noise = self._compute_conditioners(sigma)
        rescaled_obs = obs / self.cfg.sigma_data ##这里做rescale作用未知
        rescaled_noise = noisy_next_obs * c_in
        model_output = self.inner_model(rescaled_noise, c_noise, rescaled_obs, act,drone_states,obstacles) #F_theta(c_in*noised_x_t+1,x_[0:t],act_[0:t])
        denoised = model_output * c_out + noisy_next_obs * c_skip
        return model_output, denoised

    @torch.no_grad()
    def denoise(self, noisy_next_obs: Tensor, sigma: Tensor, obs: Tensor, act: Tensor,drone_states:Tensor,obstacles:Tensor) -> Tensor:
        _, d = self(noisy_next_obs, sigma, obs, act,drone_states,obstacles)
    ...
    def compute_loss(self, batch: Batch):
        ...
        for i in range(seq_length):
            ...
            act = batch.act[:, i : n + i]
            drone_states = batch.drone_state[:, i : n + i]
            obstacles = batch.obstacle_relpos[:, i : n + i]
            ...
            model_output, denoised = self(noisy_next_obs, sigma, obs, act, drone_states,obstacles)
