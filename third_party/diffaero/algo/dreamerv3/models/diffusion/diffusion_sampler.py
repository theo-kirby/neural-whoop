...
class DiffusionSampler:
    ...
    @torch.no_grad()
    def sample_next_obs(self, obs: Tensor, act: Tensor,drone_states:Tensor,obstacles:Tensor) -> Tuple[Tensor, List[Tensor]]:
        device = obs.device
        b, t, c, h, w = obs.size()
        obs = obs.reshape(b, t * c, h, w)
        s_in = torch.ones(b, device=device)
        gamma_ = min(self.cfg.s_churn / (len(self.sigmas) - 1), 2**0.5 - 1) #这里gamma_就是0？
        ...
        for sigma, next_sigma in zip(self.sigmas[:-1], self.sigmas[1:]):
            ...
            denoised = self.denoiser.denoise(x, sigma, obs, act,drone_states,obstacles)
